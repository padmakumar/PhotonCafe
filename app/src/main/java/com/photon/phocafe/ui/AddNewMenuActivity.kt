...
import kotlinx.android.synthetic.main.add_new_menu.*
...
class AddNewMenuActivity : AppCompatActivity(), EasyPermissions.PermissionCallbacks {
    private var mFirestore: FirebaseFirestore? = null
    private var mImageRef: StorageReference? = null
    var mItemUrl: String?=null

    var isNewMenu:Boolean=true

    var imagePicker: ImagePicker?=null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.add_new_menu)
        // Firestore
        mFirestore = FirebaseFirestore.getInstance()

        setSpinnerValues()

        itemAdd.setOnClickListener { if(isNewMenu){
            addNewMenuItem()
        }
        else {
            addItem()//for Todays menu
        }}
    ...
    private fun setSpinnerValues() {
        //TODO: add categories with images later in spinner
        // Create an ArrayAdapter using the string array and a default spinner layout
        val adapterCategories = ArrayAdapter.createFromResource(this,
                R.array.categories, android.R.layout.simple_spinner_item)
// Specify the layout to use when the list of choices appears
        adapterCategories.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
// Apply the adapter to the spinner
        menu_categories.setAdapter(adapterCategories)

        //set city spinner
        val adapterCity = ArrayAdapter.createFromResource(this,
...
//        mImageRef=mImageRef!!.child("phocafe")
        mImageRef!!.putFile(uri)
                ...
                /*    Toast.makeText(this@AddNewMenuActivity, "Image uploaded"+taskSnapshot.downloadUrl,
...
//                    showDownloadUI()
                ...
                .addOnFailureListener(this) { e ->
                    ...
                    Toast.makeText(this@AddNewMenuActivity, "Upload failed",
                            Toast.LENGTH_SHORT).show()
                }
    }
    ...
    /*   imagePicker= ImagePicker(this ,null)
       {imageUri -> selectImage.setImageURI(imageUri)}*/
...
//        startActivityForResult(i, RC_CHOOSE_PHOTO)
    ...
    /**
     * Add the item to the fire store to display in menu page
     */
    private fun addNewMenuItem() {
        ...
        val newMenuRef =mFirestore!!.collection("restaurants").document()
//constructor(name: String, city: String, category: String, photo: String, price: Int, numRatings: Int, avgRating: Double) {
        var price:Int=Integer.parseInt(itemPrice.text.toString())
        var categoryVal:String= menu_categories.selectedItem.toString()
        var cityVal:String = cities.selectedItem.toString()
        val tdMnu = Restaurant(itemName.text.toString(),cityVal,categoryVal, mItemUrl.toString(),price,0,0.0)

        // Add todays special
        batch?.set(newMenuRef, tdMnu)

        batch?.commit()?.addOnCompleteListener { task ->
            if (task.isSuccessful) {
                Log.d("NewMenu", "Write batch succeeded.")
//                Snackbar.make(findViewById(android.R.id.content),"Added Successful",Snackbar.LENGTH_SHORT).show()
                clearfileds()//if added success clear the fields to add new
                finish()
            } else {
                Log.w("NewMenu", "write batch failed.", task.exception)
    ...
    /**
     * clear the fields to add new item in edittext
     */
    ...
    /**
     * Add the item to the fire store to display in Todays Special page
     */
    private fun addItem() {
